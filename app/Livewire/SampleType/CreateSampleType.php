<?php

namespace App\Livewire\SampleType;

use App\Service\SampleTypeService;
use Exception;
use Illuminate\Validation\ValidationException;
use Livewire\Component;

class CreateSampleType extends Component
{
    public $name = '';

    public $description = '';

    public function save(SampleTypeService $sampleTypeService)
    {
        try {
            $data = [
                'name' => $this->name,
                'description' => $this->description,
            ];

            $sampleTypeService->createSampleType($data);

            session()->flash('success', 'Tipo de amostra criado com sucesso!');

            return redirect()->route('sample-type.index');
        } catch (ValidationException $e) {
            foreach ($e->errors() as $field => $messages) {
                foreach ($messages as $message) {
                    $this->addError($field, $message);
                }
            }
        } catch (Exception $e) {
            $this->addError('form', 'Erro ao criar tipo de amostra: '.$e->getMessage());
        }
    }

    public function render()
    {
        return view('livewire.sample-type.create-sample-type');
    }
}
